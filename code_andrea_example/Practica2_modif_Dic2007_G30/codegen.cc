...
int newLabelEq(bool inicialitzar = false){
	static int comptador = 1;
	if(inicialitzar) comptador = 0;
	return comptador++;
}
...
{
  if (isbasickind(tp->kind) || tp->kind=="pointer") {
    tp->size=4;
...
codechain GenRight(AST *a,int t)
{
 ...
 /*Es referenciable!*/
  if (a->ref) {
cout<<"MIRANDO GenRight en ident"+a->text+" ende"<<endl;
  ...
  /* IGUALTAT */
	...
	if (isbasickind(a->down->tp->kind) || isbasickind(a->down->right->tp->kind)) {
		c = GenRight(a->down,t)||
		GenRight(a->down->right,t+1)||
		"equi t"+itostring(t)+" t"+itostring(t+1)+" t"+itostring(t);
	}else{
		int lab1 = newLabelEq(false);
		int lab2 = newLabelEq(false);
		c = "iload 1 t"+itostring(t) || GenRight(a->down,t+1)||
		GenRight(a->down->right,t+2)||
		"iload "+itostring(a->down->tp->size)+" t"+itostring(t+3)||
		"etiq equ_"+itostring(lab1)||
		"fjmp t"+itostring(t)+" equ_"+itostring(lab2) || "fjmp t"+itostring(t+3)+" equ_"+itostring(lab2) ||
		"load t"+itostring(t+1)+" t"+itostring(t+4)   || "load t"+itostring(t+2)+" t"+itostring(t+5) ||
		"equi t"+itostring(t+4)+" t"+itostring(t+5)+" t"+itostring(t+4) ||
		"land t"+itostring(t)+" t"+itostring(t+4)+" t"+itostring(t) || "addi t"+itostring(t+1)+" 4 t"+itostring(t+1) ||
		"addi t"+itostring(t+2)+" 4 t"+itostring(t+2) || "subi t"+itostring(t+3)+" 4 t"+itostring(t+3) ||
		"ujmp equ_"+itostring(lab1) || "etiq equ_"+itostring(lab2);
	}
