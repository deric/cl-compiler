...
/** OPERACIONS COMPARACIO **/
	...
	{			// Igualtat, es permet bool = bool o int = int.
	    if ((a->down->tp->kind != "error" && (a->down->right->tp->kind != a->down->right->tp->kind)) ||
		(a->down->right->tp->kind != "error" && (a->down->right->tp->kind != a->down->right->tp->kind)))
...
/** PUNTER **/
      ...
      TypeCheck (a->down);		//Comprovacio del tipus

      a->tp = create_type("pointer", a->down->tp, 0);	// Creacio del tipus Punter amb el tipus del node d'entrada
...
/** ACCES A PUNTER **/
    ...
    //Si no es punter... error!
    if ((a->down->tp->kind!="error" && a->down->tp->kind!="pointer")) {

      		errorincompatibleoperator(a->line,"pointer"+a->kind);		//Error: Incompatible Operator
    }

    else {
	  if (a->down->tp->kind != "error")	a->tp = a->down->tp->down;	// Es POINTER: Assignem el tipus del pointer.
	  else	a->tp = create_type ("error", 0, 0);		// Es ERROR: Crear type d'error
	      
    }
    
    a->ref = 1;		// Operacio referencianble
...
/** REFERENCIA **/
	...
	//Si no es referenciable... error!
	if (a->down->ref != 1)		//Si el parametre no es referenciable
	{
	  errornonreferenceableexpression (a->line, a->kind);		//Error: Non Referenceable Expression
	 a->tp = create_type ("pointer", a->down->tp, 0);		// Es ERROR: Crear type d'error
	}
	else {
		  if (a->down->tp->kind != "error")	a->tp = create_type("pointer", a->down->tp,0);	// Es POINTER: Assignem el tipus del pointer.
		  else	a->tp = create_type ("error", 0, 0);		// Es ERROR: Crear type d'error
	}
