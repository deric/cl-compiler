...
void errorwrongday(int l) {
  TypeError = 1;
  cout<<"L. "<<l<<": Invalid value for day in date constant."<<endl;
}

void errorwrongmonth(int l) {
  TypeError = 1;
  cout<<"L. "<<l<<": Invalid value for month in date constant."<<endl;
}

void errorwrongyear(int l) {
  TypeError = 1;
  cout<<"L. "<<l<<": Invalid value for year in date constant."<<endl;
}
/************************************************************************************************************************************************/
/************************************************************************************************************************************************/
...
/**  IDENTIFICADOR  **/
  } else if (a->kind=="ident")
    {
	   ...
	   //   a->kind = "expident";
	    ...
	    //Si no es procedure ni funcio: SI referenciable
...
/** PARÈNTESI ( **/
	...
	if (info == "instruction")	//Es una crida a FUNCIO o PROCEDIMENT
	{
	     ...
	     else if (a->down->tp->kind == "procedure")	//Es un PROCEDURE
		{
		  ...
		  a->kind = "idproc";		// Assignem el tipus (Generacio de codi)
		}
	      else
      ...
      else	//Si no es "instruction" --> ara es "expression"
	{
	      ...
	      else if (a->down->tp->kind == "function")		//Es un FUNCTION
		{
		  ...
		  a->kind = "idfunc";		// Assignem el tipus (Generacio de codi)
...
/** DATE **/
	...
	a->tp=create_type(a->kind,0,0);
	//S'afegeix els camps del date (a l'struct)
	a->tp->struct_field["day"]=create_type("int",0,0);	
	a->tp->ids.push_back("day");
	a->tp->struct_field["month"]=create_type("int",0,0);
	a->tp->ids.push_back("month");
	a->tp->struct_field["year"]=create_type("int",0,0);	
	a->tp->ids.push_back("year");
...
/** DAY **/
    ...
    TypeCheck (a->down);		// Comprovacio del parametre 
	a->tp=create_type("int",0,0);	// Li assignem el tipus de retorn (intconst)
	a->ref = 0;	//Function NO es referenciable

	//No es una data
      if (a->down->tp->kind != "error" && (a->down->tp->kind != "date"))	errorincompatibleparam(a->line, 1);		//Error: Incompatible type
	//Si el parametre no es referenciable
     // else if (a->down->ref == 0)	errorreferenceableparam (a->line, 1);		//Error: Parameter is expected to be referenceable but it is not
...
/** SDAY **/
    ...
    TypeCheck (a->down);		// Comprovacio del primer parametre 
    TypeCheck (a->down->right);		// Comprovacio del segon parametre

	a->ref = 0;	//Procedure NO es referenciable

	//Si el primer parametre no es referenciable
      if (a->down->ref == 0)	errorreferenceableparam (a->line, 1);		//Error: Parameter is expected to be referenceable but it is not
	//No es una data
      if (a->down->tp->kind != "error" && (a->down->tp->kind != "date"))	errorincompatibleparam(a->line, 1);		//Error: Incompatible type
	
	//No es un enter
      if (a->down->right->tp->kind != "error" && (a->down->right->tp->kind != "int"))	errorincompatibleparam(a->line, 2);	//Error: Incompatible type


/** SMONTH **/
  } else if (a->kind=="smonth") {
    TypeCheck (a->down);		// Comprovacio del primer parametre 
    TypeCheck (a->down->right);		// Comprovacio del segon parametre
...
/** DATECONST **/
    ...
    TypeCheck (a->down);		// Comprovacio del dia
    TypeCheck (a->down->right);		// Comprovacio del mes
    TypeCheck (a->down->right->right);		// Comprovacio de l'any

	a->ref = 0;	//Constant NO es referenciable
	a->tp = create_type("date",0,0);
	//S'afegeix els camps del date (a l'struct)
	a->tp->struct_field["day"]=create_type("int",0,0);	
	a->tp->ids.push_back("day");
	a->tp->struct_field["month"]=create_type("int",0,0);
	...
	//Si el dia no esta en el rang
      if (atoi(a->down->text.c_str())<1 || atoi(a->down->text.c_str())>31)	errorwrongday (a->line);
	//Si el mes no esta en el rang
      if (atoi(a->down->right->text.c_str())<1 || atoi(a->down->right->text.c_str())>12)	errorwrongmonth(a->line);	
	
	//Si l'any no esta en el rang
      if (atoi(a->down->right->right->text.c_str())<1900)	errorwrongyear(a->line);	//Error: Incompatible type
...
void create_header(AST *a)
{
	...
	ptype type = a->tp;		// a1 = identificador del procedure
	...
	/*
	 a->tp->ids.push_back (paramid->text);				// Guardem l'id per mantenir l'ordre dels parametres
	 access->tp->struct_field[access->kind] = paramid->right->tp;
	 access->tp->ids.push_back (access->kind);	// Insertem el tipus del param a la llista (val/ref)
...
bool check_num_params (AST * a, ptype tp, int line)
{
	  ...
	  AST *params = a;	// Obtenim la llista de parametres de la crida (punter al primer element)

	if (params == 0 && tp == 0)	return true;
	else if (params == 0 || tp == 0)	return false;
	else return check_num_params (params->right, tp->right, line);
}
