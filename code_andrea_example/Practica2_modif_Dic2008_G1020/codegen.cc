...
// 	||"load t"+itostring(t)+" t"+itostring(t);
	...
	c = GenRight(a->down,t)
	||"addi t"+itostring(t)+" "+itostring(a->down->tp->offset[a->kind])+" t"+itostring(t)
	||"load t"+itostring(t)+" t"+itostring(t);
...
/* YEAR */
	...
	int size = compute_size(a->tp);
	c="aload aux_space t"+itostring(t)||"aload aux_space t"+itostring(t+2)||
	"addi t"+itostring(t)+" "+ itostring(offsetauxspace)+ " t"+itostring(t)||
	"iload "+a->down->text+" t"+itostring(t+1)||
	"stor t"+itostring(t+1) +" t"+itostring(t+2)||
	"addi t"+itostring(t+2)+" 4 t"+itostring(t+2)||
	"iload "+a->down->right->text+" t"+itostring(t+1)||
	"stor t"+itostring(t+1) +" t"+itostring(t+2)||
	"addi t"+itostring(t+2)+" 4 t"+itostring(t+2)||
	"iload "+a->down->right->right->text+" t"+itostring(t+1)||
	"stor t"+itostring(t+1) +" t"+itostring(t+2);
	offsetauxspace+=size;
...
codechain GenLeft(AST *a,int t)
{
  ...
  /* YEAR */
	...
	cout<<"BIG PROBLEM! No case defined in GenLeft for kind "<<a->kind+" "+a->down->text<<endl;
  }
  //cout<<"Ending with node \""<<a->kind<<"\""<<endl;
  return c;
}
...
codechain CodeGenInstruction(AST *a,string info="")
{
  ...
  /* ASSIGNACIO */
    ...
    // PART DRETA TIPUS DATE
    } else if (a->down->tp->kind=="date"){
      c=GenLeft(a->down,0)||
	GenRight(a->down->right,1)||
	"copy t1 t0 "+itostring(a->down->tp->size);
...
/* SDAY */
  } else if (a->kind=="sday") {  // call of procedure
	c=GenLeft(a->down,0)||"addi t0 "+itostring(a->down->tp->offset["day"])+" t0"||
	GenRight(a->down->right,1)||
	//"copy t1 t0 "+itostring(a->down->right->tp->size);
	"stor t1 t0";

/* SMONTH */
  } else if (a->kind=="smonth") {  // call of procedure
	c=GenLeft(a->down,0)||"addi t0 "+itostring(a->down->tp->offset["month"])+" t0"||
	GenRight(a->down->right,1)||
	//"copy t1 t0 "+itostring(a->down->right->tp->size);
	"stor t1 t0";

/* SYEAR */
  } else if (a->kind=="syear") {  // call of procedure
	c=GenLeft(a->down,0)||"addi t0 "+itostring(a->down->tp->offset["year"])+" t0"||
	GenRight(a->down->right,1)||
	//"copy t1 t0 "+itostring(a->down->right->tp->size);
	"stor t1 t0";
