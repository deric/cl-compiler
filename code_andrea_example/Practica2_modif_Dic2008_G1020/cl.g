...
constr_type: INT | STRUCT^ (field)* ENDSTRUCT! | BOOL | ARRAY^ OPENBRA! INTCONST CLOSEBRA! OF! constr_type | DATE;
...
instruction:
	IDENT^  ((OPENBRA^ expression CLOSEBRA! | DOT^ IDENT)* ASIG^ expression | OPENPAR^ list_call_param CLOSEPAR!)
	...
	|SYEAR^ OPENPAR! ( expression COMMA! expression ) CLOSEPAR!
	|SMONTH^ OPENPAR! ( expression COMMA! expression ) CLOSEPAR!
	|SDAY^ OPENPAR! ( expression COMMA! expression ) CLOSEPAR!;
...
expsimple:
	IDENT^ (OPENBRA^ expression CLOSEBRA! | DOT^ IDENT | OPENPAR^ list_call_param CLOSEPAR!)*
	|INTCONST
	|TRUe
	|FALSe
	|NOT^ expsimple
	...
	|PILLOW^ datevalue
	|YEAR^ OPENPAR! ( expression ) CLOSEPAR!
	|MONTH^ OPENPAR! ( expression ) CLOSEPAR!
	|DAY^ OPENPAR! ( expression ) CLOSEPAR!;

datevalue: INTCONST DIV! INTCONST DIV! INTCONST;
