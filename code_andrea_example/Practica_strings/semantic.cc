...
void errorprimerparam(int l) {
  TypeError = 1;
  cout<<"L. "<<l<<": Substr con primer parametro no estring."<<endl;
}
void errorsegundoparam(int l) {
  TypeError = 1;
  cout<<"L. "<<l<<": Substr con segundo parametro no estring."<<endl;
}
void errortercerparam(int l) {
  TypeError = 1;
  cout<<"L. "<<l<<": Substr con tercer parametro no estring."<<endl;
}
/************************************************************************************************************************************************/
/************************************************************************************************************************************************/
...
/** OPERACIONS ARITMETIQUES **/
  } else if (a->kind=="+" || (a->kind=="-" && a->down->right!=0) || a->kind=="*" || a->kind=="/" ||  a->kind=="div") {
    ...
    if ((a->down->tp->kind!="error" && a->down->tp->kind!=a->down->right->tp->kind) || (a->down->right->tp->kind!="error" && a->down->tp->kind!=a->down->right->tp->kind)) {

      		errorincompatibleoperator(a->line,a->kind);		//Error: Incompatible Operator
    }

    a->tp = a->down->tp;	//Tipus de l'operacio = int o bool, depenent del tipus de la part esquerra (o dreta)
...
/** OPERACIONS COMPARACIO **/
	...
	{			// Igualtat, es permet bool = bool o int = int.
	    if ((a->down->tp->kind != "error" || (a->down->right->tp->kind != "bool" && a->down->right->tp->kind != "int" && a->down->right->tp->kind != "string"))
		&& (a->down->right->tp->kind != "error" || (a->down->tp->kind != "bool" && a->down->tp->kind != "int" && a->down->tp->kind != "string")) 
		&& (a->down->tp->kind != "int" || a->down->right->tp->kind != "int")
		&& (a->down->tp->kind != "string" || a->down->right->tp->kind != "string") 
	...
	//Qualsevol comparacio excepte IGUALTAT: nomes permet int op int
		...
		|| (a->down->tp->kind != "error" && a->down->tp->kind != "string")
		|| (a->down->right->tp->kind != "error" && a->down->right->tp->kind != "string") )
	{
	  errorincompatibleoperator (a->line, a->kind);			//Error: Incompatible Operator
	}
...
/** CONSTANT STRING **/
  } else if (a->kind=="stringconst") 
    {
    a->tp=create_type("string",0,0);

/** SUBSTR (Substring) **/
  }else if (a->kind == "substr")
    {

      TypeCheck (a->down);		// Comprovacio del primer parametre 
      TypeCheck (a->down->right);		// Comprovacio del segon parametre 
      TypeCheck (a->down->right->right);		// Comprovacio del tercer parametre 


	if(a->down->tp->kind != "error" && a->down->tp->kind!="string"){
	  errorprimerparam (a->line);		//Error: Tercer Param
	}
	
	if(a->down->right->tp->kind != "error" && a->down->right->tp->kind!="int"){
	  errorsegundoparam (a->line);		//Error: Segon Param
	}	


	if( a->down->right->right!=0 && a->down->right->right->tp->kind!="error" && a->down->right->right->tp->kind!="int"){
	  errortercerparam (a->line);		//Error: Tercer Param
	}
	a->tp = create_type("string",0,0);
