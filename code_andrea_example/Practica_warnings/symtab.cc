...
infosym &symtab::operator[](string id)
{
  ...
  current->ids.push_back(id);	//Control de l'ordre d'insercio de variables 
  return current->m[id];	//Actualitzar el top de la pila (current)
}

//Recorregut dels scopes fins a trobar la variable
bool symtab::find(string id)
{
  scope *sc=current;
  
  while (sc!=0) {
    //Si el trobo: retornar true
    if (sc->m.find(id)!=sc->m.end()) {
      return true;
    }
    //No el trobo a l'scope actual: miro a l'scope anterior
    sc=sc->previous;
  }
  //Si arribo al bottom (sc==0), no l'he trobat
  return false;
}
