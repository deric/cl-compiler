...
int newLabelAnd(bool inicialitzar = false){
	static int comptador = 1;
	if(inicialitzar) comptador = 0;
	return comptador++;
}

int newLabelOr(bool inicialitzar = false){
	static int comptador = 1;
	if(inicialitzar) comptador = 0;
	return comptador++;
}

int newLabelSwitch(bool inicialitzar = false){
	static int comptador = 1;
	if(inicialitzar) comptador = 0;
	return comptador++;
}
...
/* AND */
    ...
    {
	int lab = newLabelAnd(false);
	c = GenRight(a->down,t)||
	"fjmp t"+itostring(t)+" and_"+itostring(lab)||
	GenRight(a->down->right,t+1)||
	"land t"+itostring(t)+" t"+itostring(t+1)+" t"+itostring(t)||
	"etiq and_"+itostring(lab);

/* OR */    
  } else if (a->kind == "or")
    {
	int lab = newLabelOr(false);
	c = GenRight(a->down,t)||
	"fjmp t"+itostring(t)+" or_"+itostring(lab)||
	"ujmp endor_"+itostring(lab)||
	"etiq or_"+itostring(lab)||	
	GenRight(a->down->right,t+1)||
	"loor t"+itostring(t)+" t"+itostring(t+1)+" t"+itostring(t)||
	"etiq endor_"+itostring(lab);
...
/* SWITCH */
        ...
        c="etiq switch_"+itostring(switchlab)||GenRight(a->down,0)||	//Evaluación de la expresión
	CodeGenInstruction(a->down->right,"instruction")||
	"etiq endswitch_"+itostring(switchlab);

	switchlab++;

/* CASE */
  } else if (a->kind=="case") {
	int lab = newLabelSwitch(false);

	c=GenRight(a->down,1)||
	"equi t0 t1 t2"||
	"fjmp t2 next_"+itostring(lab)||
	CodeGenInstruction(a->down->right,"instruction")||
	"ujmp endswitch_"+itostring(switchlab)||
	"etiq next_"+itostring(lab);
