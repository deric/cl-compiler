...
/** ACCES A ARRAY ([) **/
	...
	{
	  if (a->down->right->right != 0)  errorincompatibleoperator (a->line, "array[:]");
	  else  errorincompatibleoperator (a->line, "array[]");	//Error: Incompatible Operator
      ...
      if (a->down->right->tp->kind != "error" && a->down->right->tp->kind != "int")	//L'index no es "int"
	{
	  errorincompatibleoperator (a->line, "[]");		//Error: Incompatible Operator
       ...
       if (a->down->right->right != 0 && a->tp->kind != "error")	//Fem access segmentat
	{
		if (a->down->right->kind != "intconst")		errornotanintconst(a->line, "first");
		else	{
			if (atoi(a->down->right->text.c_str()) < 0 
				|| atoi(a->down->right->text.c_str()) >= a->down->tp->numelemsarray){
				erroroutrange(a->line, "lower");
			}else{
				if (atoi(a->down->right->text.c_str())>atoi(a->down->right->right->text.c_str())) {
					errorinvalidrange(a->line);
				}
			}
		}
		if (a->down->right->right->kind != "intconst")		errornotanintconst(a->line, "second");
		else	{
			if (atoi(a->down->right->right->text.c_str()) >= a->down->tp->numelemsarray)	erroroutrange(a->line, "upper");
...
void create_header(AST *a)
{
	...
	AST *a1 = a->down;		// a1 = identificador del procedure
  	...
  	AST *access = param->down;	// access = tipus del parametre (idparref/idparval)

	if (access != 0)	// tipus de parametre no es nul
	    {

		access->tp = create_type ("", 0, 0);	// Creem l'estructura del tipus d'un parametre
		a->tp->down = access->tp;		// Enllaç amb el tipus que acabem de crear
 
		while (access != 0)		//Mentre hi hagi parametres per revisar
			{
		  	AST *paramid = access->down;	// paramid = Identificador del parametre
		  	TypeCheck (paramid->right);	// TypeCheck del tipus del parametre
		  	a->tp->struct_field[paramid->text] = paramid->right->tp;	// Guardem a un map tots cadascun dels parametres
			...
			if (access->right != 0)
			{
				access->right->tp = create_type ("", 0, 0);	// Creem l'estructura del proxim parametre
				access->tp->right = access->right->tp;		// Creem l'enllaç a aquest proxim (seguent) parametre
		   	 }
			access = access->right;		//Passem al seguent parametre
		}
	    }
	  if (a->kind == "function")
	    {				// La capsalera es una funcio i hem de guardar el tipus
	      ptype tp;
	      TypeCheck (param->right);	// TypeCheck del tipus de retorn
	      tp = param->right->tp;
	      a->tp->right = tp;	// Li assignem el tipus de retorn
	    }
...
void check_params(AST *a)
{
  ...
  if (ok)	//Coincideix. Fem Comprovacio de tipus
      ...
      while (aux != 0 && t != 0)	//Bucle per recorrre els parametres
	{
	  ...
	  aux = aux->right;	// Avancem al seguent parametre
...
void check_param (AST * a, ptype tp, int line, int numparam)
{
  ...
  AST *a1 = a;			//Copia que conte els parametres de la crida
