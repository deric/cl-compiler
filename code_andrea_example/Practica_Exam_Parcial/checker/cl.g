...
instruction:
	IDENT^  ((OPENBRA^ expression ( COLON! expression|) CLOSEBRA! | DOT^ IDENT)* ASIG^ expression | OPENPAR^ list_call_param CLOSEPAR!)
...
expsimple:
	IDENT^ (OPENBRA^ expression ( COLON! expression|)  CLOSEBRA! | DOT^ IDENT | OPENPAR^ list_call_param CLOSEPAR!)*
	|INTCONST
	|TRUe
	|FALSe
	|NOT^ expsimple
