...
public:
  ...
  int &operator[](int i) {
    if (m.find(i)==m.end()) {
      ...
      outputexecution<<"Execution error: out of memory access."<<endl;
      exit(1);
    }
    return m[i];
  }
  void executepush(int value) {
    ...
    if (stacksize>=MAXSTACK) {
      ...
      outputexecution<<"Execution error: stack size used is bigger than the allowed size."<<endl;
      exit(1);
    }
  }
  ...
  void free(int memposition) {
    ...
    if (allocatedsize.find(memposition)==allocatedsize.end()) {
      ...
      outputexecution<<"Execution error: free of a non-allocated memory position."<<endl;
      exit(1);
    }
	   ...
	   map<string,int> &temporal)
{
  if (isint(variablename)) {
    ...
    outputexecution<<"EXECUTION ERROR: expression "<<variablename<<" has not a value."<<endl;
    exit(1);
  }
}
...
int executesubroutine(codesubroutine *cs)
{
  ...
  for (codenode *instruction=cs->c.first;
    ...
    if (--timelimit==0) {
      ...
      outputexecution<<endl<<"Time limit exceeded: the execution is aborted"<<endl;
      return 1;
    }    
	...
	value(scopebase,cs->name,instruction->op[1],temporal);
      ...
      outputexecution<<value(scopebase,cs->name,instruction->op[0],temporal);
    } else if (instruction->codop=="wris") {
      cout<<instruction->op[0].substr(1,int(instruction->op[0].size())-2);
      outputexecution<<instruction->op[0].substr(1,int(instruction->op[0].size())-2);
    } else if (instruction->codop=="wrln") {
      cout<<endl;
      outputexecution<<endl;
    } else {
      cout<<endl<<"EXECUTION ERROR: non existing instruction "<<instruction->codop<<endl;
      outputexecution<<endl<<"EXECUTION ERROR: non existing instruction "<<instruction->codop<<endl;
      return 1;
    }
  }
  cout<<endl<<"Error, list of instructions does not end with stop or retu"<<endl;
  outputexecution<<endl<<"Error, list of instructions does not end with stop or retu"<<endl;
  return 1;
}
...
int executecodeglobal(codeglobal &cg,int executiontime,int doitstepbystep)
{
  ...
  outputexecution.open("outputexecution");
  int result=executesubroutine(&cg.mainsub);
  outputexecution.close();
  return result;
}
